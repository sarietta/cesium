...
extern "C" {
  ...
  void F77NAME(dneupd)(ARlogical *rvec, char *HowMny, ARlogical *select,
		       ...
		       double *sigmai, double *workev, char *bmat,
		       ARint *n, char *which, ARint *nev,
		       double *tol, double *resid, ARint *ncv,
		       double *V, ARint *ldv, ARint *iparam,
...
namespace slib {
  namespace matrix {
    ...
    int EigenSolver::eigs(const DoubleMatrix& A, const int& neigs, 
    ...
    int EigenSolver::eigs(const double* A, const int& N,
      ...
      ARint nev = neigs;
      double tol = -1.0f;
      scoped_array<double> resid(new double[n]);
      for (int i = 0; i < n; i++) {
	resid[i] = Random::Uniform(0.0f, 1.0f);
      }
      ARint ncv = 2*nev;
      scoped_array<double> V(new double[n * ncv]);
      memset(V.get(), 0, sizeof(double) * n * ncv);
      ARint ldv = n;
      scoped_array<ARint> iparam(new int[11]);
      memset(iparam.get(), 0, sizeof(ARint) * 11);
      {
	...
	iparam[6] = 1;
      ...
      scoped_array<ARint> ipntr(new int[14]);
      memset(ipntr.get(), 0, sizeof(ARint) * 14);
      scoped_array<double> workd(new double[3 * n]);
      memset(workd.get(), 0, sizeof(double) * 3 * n);
      ARint lworkl = 3*ncv*(ncv+2);  // See documentation
      scoped_array<double> workl(new double[lworkl]);
      ARint info_dnaupd = 1;
	...
	// Repeatedly call the routine DNAUPD and take actions indicated by parameter IDO until
	// either convergence is indicated or maxitr has been exceeded.        
	F77NAME(dnaupd)(&ido, bmat, &n, which, &nev, &tol, resid.get(), 
			&ncv, V.get(), &ldv, iparam.get(), ipntr.get(), workd.get(), workl.get(), 
			&lworkl, &info_dnaupd);
	...
	if (info_dnaupd != 0) {    
	  ...
	  if (info_dnaupd == 1) {
	    ...
	    LOG(WARNING) << "Error with dnaupd, info = " << info_dnaupd;
	    LOG(WARNING) << ">> Check the documentation of dnaupd";
	  }
	  if (info_dnaupd == -9999) {
	    LOG(WARNING) << "Size of current Arnoldi factorization: " << iparam[4];
	  }
	...
	return info_dnaupd;
      ...
      char howmany[1] = {'A'};
      scoped_array<ARlogical> select(new ARlogical[ncv]);
      scoped_array<double> dr(new double[nev + 1]);
      scoped_array<double> di(new double[nev + 1]);
      double sigmar;
      double sigmai;
      scoped_array<double> workev(new double[3 * ncv]);
      ARint info_dneupd;
      
      info_dneupd = 0;
      
      F77NAME(dneupd)(&rvec, howmany, select.get(), dr.get(), di.get(), V.get(), &ldv, 
		      &sigmar, &sigmai, workev.get(), 
		      bmat, &n, which, &nev, &tol, resid.get(), 
		      &ncv, V.get(), &ldv, iparam.get(), ipntr.get(), workd.get(), workl.get(), 
		      &lworkl, &info_dneupd);
      ...
      if (info_dneupd != 0) {
	LOG(WARNING) << "Error with dneupd, info = " << info_dneupd;
	LOG(WARNING) << ">> Check the documentation of dneupd.";
      }
